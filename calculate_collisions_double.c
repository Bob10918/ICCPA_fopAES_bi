...
void read_traces_d(FILE* input, char plaintexts[M][plaintextlen]);
char** analyze_traces_d();
...
void* find_colisions_d(void* args);
...
/*
 * Calculate collision_ds starting from the power traces
 */
char** calculate_collisions_double(FILE* infile){
    ...
    read_traces_d(infile, m);
    
    fclose(infile);    
    
    return analyze_traces_d();
...
/*
 * Compute covariance_d of given samples in the given time sequences
 */
DATA_TYPE covariance_d(DATA_TYPE** T, int theta0, int theta1, int t, DATA_TYPE* sum_array){
    int i;
...
char** analyze_traces_d(){
    ...
    for(int i=0; i<KEY_SIZE; i++){
        ...
        args->T = T;
        pthread_create(&(running_threads[i]), NULL, find_colisions_d, (void* ) args);
        //find_colisions_d((void*)args);
    }
...
/*
 * Check collision_ds inside a given key byte
 */
void* find_colisions_d(void* args){
    ...
    for(i=0; i<(BYTE_SPACE/2); i++){    
        compute_arrays_d(T[i], sum_array, std_dev_array);
        if(collision_d(T[i], 0, l, sum_array, std_dev_array)){
            /* if a collision_d is detected, the corresponding byte value is returned
             */
